list.theFeatFiles = function(data_path, file_pattern_to_find, dataset_type) {
  
  files = list.files(path=data_path, pattern=file_pattern_to_find, recursive=TRUE)
  ...
  files_fullpath = list.files(path=data_path, pattern=file_pattern_to_find, recursive=TRUE, full.names = TRUE)
